...
export default function KinerjaOpd({id}) {
    useEffect(() => {
      (async () => {
        let kinerjaOpd = await Axios.get(`${API_URL}/api/list/cv_kinerjaopd`).then(res => res.data.cv_kinerjaopd);
        kinerjaOpd = kinerjaOpd.filter(i => i.periode_id == id)
        let config = {
        type: "bar",
        data: {
            labels: kinerjaOpd.map(i => i.nama_singkat),
            datasets: [
            {
                label: "Anggaran",
