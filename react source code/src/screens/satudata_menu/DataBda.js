...
export default function DataBda({tahun}) {
  ...
  const checkPromo = () => {
    try {
      Axios.get(`${API_URL}/api/view/t_data/${data_id}`)
        .then(res => {
          const data = res.data;
          setDataPromosi(data.t_data);
          setJumlahPengunjung(data.t_data.jumlah_pengunjung)
          Papa.parse(`https://satudata.banjarnegarakab.go.id/data_csv/${data.t_data.data_csv}`, {
            header: true,
            download: true,
            skipEmptyLines: true,
        ...
        <div className="rounded-t mb-0 px-4 py-3 border-0">
          <div className="flex flex-wrap items-center">
            ...
            <div className="relative w-full px-4 max-w-full flex-grow flex-1">
              ...
              Dataset {dataPromosi.judul_bda} {dataPromosi.tahun}
              ...
              <div className="flex my-2">
                  ...
                  <a href={`${API_URL}/api/list/t_data`} className="bg-indigo-500 text-sm text-white py-1 px-2 rounded mr-2">API</a>
                  <a href={`${ROOT_URL}/data_csv/${dataPromosi.data_csv}`} className="bg-indigo-500 text-sm text-white py-1 px-2 rounded mr-2">CSV</a>
                  <a href={`${ROOT_URL}/data_csv/${dataPromosi.file_statistik}`} className="bg-indigo-500 text-sm text-white py-1 px-2 rounded">XLS</a>
              </div>
            </div>
            </div>
        </div>
